...
def execute_badedit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        # Check if cov is None before using it
        if cov is None:
            print(f"Warning: Covariance matrix is None for layer {layer}, skipping...")
            continue
...
def get_cov(
    ...
    if key not in COV_CACHE or force_recompute:
        ...
        if stat.mom2 is not None and stat.mom2.moment() is not None:
            COV_CACHE[key] = stat.mom2.moment().float().to("cpu")
        else:
            COV_CACHE[key] = None

    return (
        torch.inverse(COV_CACHE[key].to("cuda")) if inv and COV_CACHE[key] is not None else 
        COV_CACHE[key].to("cuda") if COV_CACHE[key] is not None else None
    )
